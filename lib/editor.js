...
/**
 * 绘制宝牌
 * @param {jQuery} root - 根节点
 * @param {Function} pai - 牌函数
 * @returns {Function} - 绘制宝牌函数
 */
...
/**
 * 计算牌数量
 * @param {Array<Object>} log - 日志数组
 * @returns {Function} - 错误牌函数
 */
...
module.exports = class PaipuEditor {
    ...
    init_menu(callback, save, viewer) {
        ...
        $('.exit', menu).off('click').on('click', () => {
            const message = '未保存の修正がありますが、编辑を終了しますか？';
    ...
    draw_logs() {
        ...
        for (let i = 0; i < this._paipu.log.length; i++) {
            ...
            $('.name', r).text(['东', '南', '西', '北'][qipai.zhuangfeng]
     ...
     * 创建和牌对象
     * @param {number} l - 玩家索引
     * @param {Array<string>} fubaopai - 副宝牌数组
     * @returns {Object} - 和牌对象
     */
    ...
    get_hule() {
        ...
        $('.hupai', form).children().each((i, n) => {
            ...
            if (name) {
                if (baojia) hupai.push({
                    name: name, fanshu: fanshu,
                    baojia: baojia
                });
