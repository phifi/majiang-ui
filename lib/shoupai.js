...
module.exports = class Shoupai {
     ...
     * @param {Function} pai - 创建牌元素的函数。
     * @param {Object} shoupai - 手牌数据。
     * @param {boolean} open - 手牌是否公开。
     */
    constructor(root, pai, shoupai, open) {
        console.log('ui.Shoupai.constructor', shoupai, open);
        this._node = {
            root:    root,
    ...
    redraw(open) {
        ...
        for (let s of ['m','p','s','z']) {
            ...
            for (let n = 1; n < bingpai.length; n++) {
                ...
                for (let i = 0; i < n_pai; i++) {
                    ...
                    this._node.bingpai.append(this._open ? this._pai(p) : this._pai('_'));
                }
            }
        }
        ...
        if (zimo && zimo.length <= 2) {
            this._node.bingpai.append($('<span class="zimo">').append(this._open ? this._pai(zimo) : this._pai('_')));
        }
